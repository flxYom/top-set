...
read access to the client's logbook) and the support message thread — one
thread per person, the same from both sides, with sender-aligned bubbles, an
automatic read receipt on open, a waiting bubble while no reply has come, and
Enter to send.
